...
function AppContent() {
  ...
  // --- AUTO-REFRESH LOGIC (Fix for Stale App/404 on Resume) ---
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        ...
        // If app was in background longer than threshold, refresh to prevent stale state
        if (timeDiff > REFRESH_THRESHOLD_MS) {
          console.log("App stale, refreshing...");
  ...
  return (
    <div className={`min-h-screen bg-gray-50 ${isFullScreen ? 'overflow-hidden' : ''}`}>
       ...
       <div className={`${shouldHideHeader ? "hidden" : ""} sticky top-0 z-50 bg-white border-b border-slate-50 transition-all`}>
            ...
            {/* Mobile Header (Fixed iOS Spacing & Continuity) */}
            <div 
                className="md:hidden px-6 pb-3 flex justify-between items-center bg-white" 
                style={{ 
                    paddingTop: 'calc(env(safe-area-inset-top) + 12px)', // Consistent top padding
                    minHeight: '80px' // Ensure standard height
                }}
            >
       ...
       {/* MAIN CONTENT WRAPPER (Fixed iOS Spacing) */}
       <main className={
           isFullScreen 
           ? "w-full h-full p-0 m-0 fixed inset-0 z-[9998]" // Full Screen Mode
           : shouldHideHeader 
             ? "w-full pb-40 px-0 pt-[env(safe-area-inset-top)] md:pt-8" // Increased pb-40 to clear bottom nav
             : "px-4 md:px-8 max-w-7xl mx-auto pt-4 pb-40" // Standard Mode
       }>
          <Routes>
       ...
       {/* MOBILE TAB BAR (Fixed iOS Home Indicator Overlap) */}
       {!isSearching && !isPlateScanning && !location.pathname.startsWith('/restaurant') && location.pathname !== '/scanner' && !location.pathname.startsWith('/recipe/') && location.pathname !== '/subscription' && (
           <div 
             className="md:hidden fixed bottom-0 left-0 w-full z-50 bg-white border-t border-slate-100"
             style={{ 
                 paddingBottom: 'env(safe-area-inset-bottom)', // Pushes content up above home bar
                 height: 'calc(65px + env(safe-area-inset-bottom))' // Ensures background covers the area
             }}
           >
              <div className="flex justify-between items-center px-2 h-[65px] relative">
                  <button onClick={() => navigate('/')} className="flex flex-col items-center justify-center w-1/5 h-full pt-1">
                  ...
                  <button onClick={() => navigate('/meal-hub')} className="flex flex-col items-center justify-center w-1/5 h-full pt-1">
                  ...
                  <div className="w-1/5 relative h-full flex justify-center items-center pointer-events-none">
                      <button onClick={() => setScannerMenuOpen(true)} className="pointer-events-auto absolute -top-6 w-14 h-14 bg-slate-900 rounded-full flex items-center justify-center text-white shadow-lg shadow-slate-300">
                          <ColoredIcon src={scannerIcon} colorClass="bg-white" sizeClass="w-6 h-6" />
                      </button>
                  </div>
                  <button onClick={() => navigate('/blog')} className="flex flex-col items-center justify-center w-1/5 h-full pt-1">
                  ...
                  <button onClick={() => navigate('/account')} className="flex flex-col items-center justify-center w-1/5 h-full pt-1">
